...
int main(int argc, char **argv) {
  ...
  std::thread visual_thread(&XCHUSlam::visualThread, map);   // 发布关键帧和保存地图
  std::thread loop_thread(&XCHUSlam::loopClosureThread, map);  // 回环优化进程
...
XCHUSlam::XCHUSlam(ros::NodeHandle nh, ros::NodeHandle pnh) : nh_(nh), pnh_(pnh) {
  ...
  sub_pc_ =
      nh_.subscribe<sensor_msgs::PointCloud2>(_lidar_topic.c_str(), 100, boost::bind(&XCHUSlam::pcCB, this, _1));
...
void XCHUSlam::process_cloud(const pcl::PointCloud<PointT> &tmp, const ros::Time &current_scan_time) {
