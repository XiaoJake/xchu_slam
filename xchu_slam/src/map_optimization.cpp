...
#include "xchu_slam/map_optimization.h"

int main(int argc, char **argv) {
  ros::init(argc, argv, "map_node");

  ros::NodeHandle nh, pnh("~");
  MapOptimization *map = new MapOptimization(nh, pnh);

  std::thread visual_thread(&MapOptimization::visualThread, map);   // 发布关键帧和保存地图
  std::thread loop_thread(&MapOptimization::loopClosureThread, map);  // 回环优化进程
...
MapOptimization::MapOptimization(ros::NodeHandle nh, ros::NodeHandle pnh) : nh_(nh), pnh_(pnh) {
  // 初始化gtsam参数
  if (!init()) {
    exit(-1);
  }
  ...
  pub_predict_pose_ = nh_.advertise<nav_msgs::Odometry>("/predict_odom", 1); //  初始位姿态估计
  pub_updated_pose_ = nh_.advertise<nav_msgs::Odometry>("/updated_odom", 1); //  优化之后的pose
  ...
  sub_pc_ =
      nh_.subscribe<sensor_msgs::PointCloud2>(_lidar_topic.c_str(), 100, boost::bind(&MapOptimization::pcCB, this, _1));
  sub_odom_ = nh_.subscribe<nav_msgs::Odometry>("/updated_odom", 10, boost::bind(&MapOptimization::odomCB, this, _1));

}

bool MapOptimization::init() {
  // 初始化
...
void MapOptimization::visualThread() {
  ros::Rate rate(0.2);
  while (ros::ok()) {
    rate.sleep();
 ...
 *
 */
void MapOptimization::TransformAndSaveFinalMap() {
  std::stringstream ss;
 ...
 *
 */
void MapOptimization::adjustDistortion(pcl::PointCloud<PointT>::Ptr &cloud, double scan_time) {
 ...
 *
 */
void MapOptimization::extractLocalmap() {
  // 没有关键帧
...
void MapOptimization::process_cloud(const pcl::PointCloud<PointT> &tmp, const ros::Time &current_scan_time) {
...
void MapOptimization::odomCB(const nav_msgs::OdometryConstPtr &msg) {
  // 里程计数据的队列长度也是100
...
void MapOptimization::pcCB(const sensor_msgs::PointCloud2ConstPtr &msg) {
  std::lock_guard<std::mutex> lock(mtx_);
...
void MapOptimization::pcCB2(const sensor_msgs::PointCloud2ConstPtr &msg) {
  std::lock_guard<std::mutex> lock(mtx_);
  mutex_lock.lock();
  cloudBuf.push(*msg);
  mutex_lock.unlock();
...
void MapOptimization::imuCB(const sensor_msgs::ImuConstPtr &msg) {
  //  取姿态，是ENU数据吗？
  double roll, pitch, yaw;
